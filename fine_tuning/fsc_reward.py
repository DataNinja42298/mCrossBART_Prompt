...
SUPPORTED_LMS = ['distilgpt2', 'gpt2', 'gpt2-medium',
                 'gpt2-large', 'gpt2-xl', 'mbart-large-cc25', 'csebuetnlp/mT5_m2m_crossSum_enhanced']
...
class PromptedTextStyleTransferReward(BaseReward):
    def __init__(
        ...
        self.generator = PromptedGenerator(task_lm, template, end_punct,                                    # 基于提示生成
        ...
        self.selector = TextStyleTransferOutputSelector(style_tokenizer,                                    # 计算reward
                                                        style_batch_size,
                                                        reward_device)
    ...
    def forward(
        ...
        if mode == 'train':
            self._counter += 1
            source_strs = source_texts # self._repeat_texts(source_texts)
            target_labels = target_labels # self._repeat_texts(target_labels)
        elif mode == "infer":
        ...
        # print(len(prompt_strs))
        # print(len(source_strs))
        ...
        for i, (prompt, src, label) in enumerate(zip(prompt_strs,
            ...
            averages = [sum(row) / len(row) for row in zip(sum_rewards, content_scores, style_probs)]

            # Bootstrap the max reward for k times and average
            bootstrap_max_rewards: List[float] = \
                self._boostrap_max_rewards_k_times(averages, k_reward)
            ...
            quantities_to_log["top_rouge"].append(top_rouge)

            print(self._counter, '|', prompt_tokens[i], '|',
                  prompt, '|', src, '|', hypos[top_index], '|',
                  'Top rougel:', round(top_content.item(), 2), '|',
                  'Top rouge2:', round(top_rouge.item(), 2), '|',
                  'Top rouge1:', round(top_prob.item(), 2), '|',
                  'Top Reward:', round(max_reward, 2), '|',
                  'Reward:', round(reward.item(), 2))
            rewards.append(reward)

            data += [
                [hypos[top_index], round(top_content.item(), 2), round(top_rouge.item(), 2), round(top_prob.item(), 2)],
            ]

        if self._counter % 10 == 0:
            filepath = f'{self._counter}.csv'
            self.save_csv(filepath, data)
    ...
    def save_csv(self, file_name, data):  # 将数据写入文本文件
        # 将数据写入CSV文件
